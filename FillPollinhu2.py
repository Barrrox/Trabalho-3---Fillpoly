...
class AplicacaoRenderizacao:
    ...
    def _aplicar_algoritmo_scanline(self, poligono: Poligono):
        ...
        for i in range(n):
            ...
            m_inverso_aresta = (x2 - x1) / (y2 - y1)
            y_min_aresta = int(y1)
            
            if y_min_aresta not in tabela_arestas:
                tabela_arestas[y_min_aresta] = []
                
            tabela_arestas[y_min_aresta].append({
                'y_max': y2,
                'x_inicial': x1,
                'm_inverso': m_inverso_aresta
            })

        #processamento por scanline
        arestas_ativas = []
        y_min = min(tabela_arestas.keys()) if tabela_arestas else 0
        y_max = max([edge['y_max'] for edges in tabela_arestas.values() for edge in edges]) if tabela_arestas else 0
        ...
        for y in range(int(y_min), int(y_max) + 1):
            ...
            arestas_ativas.sort(key=lambda edge: edge['x_inicial'])

            # preenche entre pares de arestas
            for i in range(0, len(arestas_ativas), 2):
                if i + 1 < len(arestas_ativas):
                    x_inicio = int(arestas_ativas[i]['x_inicial'])
                    x_fim = int(arestas_ativas[i + 1]['x_inicial'])
                    self._canvas.create_line(
                        x_inicio, y, x_fim, y,
